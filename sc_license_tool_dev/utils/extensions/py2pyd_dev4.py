...
def py_to_pyd(py_file, output_dir):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # print(f">>> Making path...{output_pyd_file}")
        shutil.copy2(pyd_file, output_pyd_file)
        # import subprocess
        # subprocess.run(['cp', pyd_file, output_pyd_file])
        # print(f">>>coppied")
...
def on_run():
    py_to_pyd(input_path.get(), output_path.get())
...
# 创建和放置窗口组件
Label(root, text="Input Python File:").grid(row=0, column=0, sticky='w')
